...
class ResNet152Classifier(BaseImgClassificationNeuralNetwork):
    ...
    def get_hyperparameter_search_space(dataset_properties=None, optimizer='smac'):
        if optimizer == 'smac':
            ...
            space = {'batch_size': hp.choice('resnet152_batch_size', [16, 32]),
                     'optimizer': hp.choice('resnet152_optimizer',
                                            [("SGD", {'sgd_learning_rate': hp.loguniform('resnet152_sgd_learning_rate',
                                                                                         np.log(1e-5), np.log(1e-2)),
                                                      'sgd_momentum': hp.uniform('resnet152_sgd_momentum', 0, 0.9)}),
                                             ("Adam",
                                              {'adam_learning_rate': hp.loguniform('resnet152_adam_learning_rate',
                                                                                   np.log(1e-5), np.log(1e-3)),
                                               'beta1': hp.uniform('resnet152_beta1', 0.5, 0.999)})]),
                     'epoch_num': 100,
                     'lr_decay': 0.8,
                     'step_decay': 10
                     }
