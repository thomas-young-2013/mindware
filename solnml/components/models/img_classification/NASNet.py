...
class NASNetClassifier(BaseImgClassificationNeuralNetwork):
    ...
    def fit(self, X, y, sample_weight=None):
        from .nn_utils.nasnet import NASNetALarge
    ...
    def get_hyperparameter_search_space(dataset_properties=None, optimizer='smac'):
        if optimizer == 'smac':
            ...
            space = {'learning_rate': hp.loguniform('nasnet_learning_rate', np.log(1e-5), np.log(1e-3)),
                     'batch_size': hp.choice('nasnet_batch_size', [8, 16, 32]),
                     'beta1': hp.loguniform('nasnet_beta1', 0.5, 0.999),
                     'epoch_num': 100,
                     'lr_decay': 10,
                     'step_decay': 10
                     }
