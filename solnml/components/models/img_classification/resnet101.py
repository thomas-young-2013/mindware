...
class ResNet101Classifier(BaseImgClassificationNeuralNetwork):

    def fit(self, dataset, **kwargs):
        from .nn_utils.pytorch_zoo_model import resnet101
        if self.grayscale:
            raise ValueError("Models from pytorch-model zoo only support RGB inputs!")
        self.model = resnet101(num_classes=len(dataset.train_dataset.classes), pretrained='imagenet')
    ...
    def set_empty_model(self, dataset):
        ...
        if self.grayscale:
            raise ValueError("Models from pytorch-model zoo only support RGB inputs!")
        self.model = resnet101(num_classes=len(dataset.classes), pretrained=None)

    @staticmethod
    def get_properties(dataset_properties=None):
        return {'shortname': 'ResNet101',
                'name': 'ResNet101 Classifier',
    ...
    def get_hyperparameter_search_space(dataset_properties=None, optimizer='smac'):
        if optimizer == 'smac':
            ...
            space = {'batch_size': hp.choice('resnet101_batch_size', [16, 32]),
                     'optimizer': hp.choice('resnet101_optimizer',
                                            [("SGD", {'sgd_learning_rate': hp.loguniform('resnet101_sgd_learning_rate',
                                                                                         np.log(1e-5), np.log(1e-2)),
                                                      'sgd_momentum': hp.uniform('resnet101_sgd_momentum', 0, 0.9)}),
                                             ("Adam",
                                              {'adam_learning_rate': hp.loguniform('resnet101_adam_learning_rate',
                                                                                   np.log(1e-5), np.log(1e-3)),
                                               'beta1': hp.uniform('resnet101_beta1', 0.5, 0.999)})]),
                     'epoch_num': 100,
                     'lr_decay': 0.8,
                     'step_decay': 10
                     }
