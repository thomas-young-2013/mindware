...
class ResNeXtClassifier(BaseImgClassificationNeuralNetwork):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.model = ResNeXt101_32x4d(num_classes=len(set(y)), grayscale=True if X.shape[1] == 1 else False)
    ...
    def get_hyperparameter_search_space(dataset_properties=None, optimizer='smac'):
        if optimizer == 'smac':
            ...
            space = {'learning_rate': hp.loguniform('resnext_learning_rate', np.log(1e-5), np.log(1e-3)),
                     'batch_size': hp.choice('resnext_batch_size', [8, 16, 32]),
                     'beta1': hp.loguniform('resnext_beta1', 0.5, 0.999),
                     'epoch_num': 100,
                     'lr_decay': 10,
                     'step_decay': 10
                     }
