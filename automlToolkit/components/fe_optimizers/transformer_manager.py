...
from automlToolkit.components.feature_engineering.transformations import _transformers, _type_infos
from automlToolkit.components.utils.configspace_utils import sample_configurations
...
class TransformerManager(object, metaclass=abc.ABCMeta):
    ...
    def get_hyperband_transformations(self, node: DataNode, trans_types: typing.List,
                                      batch_size: int = 1):
        feat_type = list(set(node.feature_types))
        if isinstance(feat_type, str):
            feat_type = [feat_type]
        ...
        for id in trans_ids:
            ...
            len_trans = len(sampled_configs)
            sample_cnt.append(len_trans)
            for config in sampled_configs:
                _transformer = transformer_class(**config.get_dictionary())
                transformers.append(_transformer)

            self.hyper_configs[id].extend(sampled_configs)
        return transformers, sample_cnt
